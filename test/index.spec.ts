...
describe("Test Command classes", () => {
  ...
  it("should set cli option of Command instance", async () => {
    ...
    const commandBuilder = await cli({
      ...
      testArguments: [
        commandKey,
        "--option-test-command-8",
        commandOptionValue,
      ],
    });
    const command: TestCommand8 = commandBuilder.initedCommands[0]
      .command as TestCommand8;

    expect(command.option1).toBe(commandOptionValue);
    expect(consoleMessages[0]).toBe(
      `I'm the testCommand ${commandKey}. option-test-command-8: ${commandOptionValue}`
    );
  });
  ...
  it("should set cli argument of Command instance", async () => {
    ...
    const command: TestCommand4 = commandBuilder.initedCommands[0]
      .command as TestCommand4;
  ...
  it("should set externally defined cli option default value to Command instance's field", async () => {
    ...
    const command: TestCommand6 = commandBuilder.initedCommands[0]
      .command as TestCommand6;
  ...
  it("should reach aggregated command fields", async () => {
    ...
    const commandBuilder = await cli({
      ...
      testArguments: [
        ...
        cliPath[1],
        aggregatedArgument1,
        `--option-test-command-4`,
      ],
    });
    const command: TestCommand12 = getCommandInstance(
      commandBuilder.initedCommands,
      cliPath
    ) as TestCommand12;

    expect(commandBuilder.runnable?.name).toBe("testCommand12");
    expect(commandBuilder.runnable?.aggregatedRelation?.name).toBe(
      "testCommand4"
    );
    expect(consoleMessages[0]).toBe(`I'm the testCommand ${cliPath[1]}`);
    expect(consoleMessages[1]).toBe(
      `Aggregated relation "argument1" value: ${aggregatedArgument1}`
    );
    expect(consoleMessages[2]).toBe(
      `Aggregated relation "option1" value: true`
    );
  });
  ...
  describe("should allow either one of the defined contained params", () => {
    ...
    it("check only with the first param", async () => {
      ...
      expect(consoleMessages[1]).toBe(
        `Aggregated relation "option1" value: ${cliCommand1[2]}`
      );
      expect(consoleMessages[2]).toBe(
        `Aggregated relation "option2" value: undefined`
      );
    });
    ...
    it("check only with the second param", async () => {
      ...
      expect(consoleMessages[1]).toBe(
        `Aggregated relation "option1" value: undefined`
      );
      expect(consoleMessages[2]).toBe(
        `Aggregated relation "option2" value: ${cliCommand2[2]}`
      );
    });
