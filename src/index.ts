...
import { Aggregation, findFirstAggregration, getAggregation } from './aggregation'
...
class CommandBuilder {
    ...
    private initCommandClasses(argv: string[], commands: { new (): Command }[]) {
        ...
        if (context.exitReason || typeof context === 'string' || !context.command?.meta?.instance) {
            return
        }
        ...
        if (context.sibling) {
            const key: string = context.command.meta.instance.siblingProperty as string
            const siblingCommand: Command = context.sibling.command.meta.instance as Command
            Reflect.set(command, key, siblingCommand)
            initCommandFields(context.sibling.command.meta.instance as Command, { ...context.sibling.options, ...context.sibling.arguments })
        }
    ...
    private createGroup(command: GroupCommand): Argv.Group {
        ...
        for (const SubcommandClass of command.subCommandClasses) {
            ...
            const parsableCommand = this.createCommand(initedSubcommand, commandOptions, commandArguments)
    ...
    private initCommandInstance(parser: Argv.Parser, initedCommand: InitedCommand): any {
        ...
        if (isGroupCommand(commandInstance)) {
            ...
            const command = this.createCommand(commandInstance, commandOptions, commandArguments)
    ...
    private createCommand(command: Command, commandOptions: IOption[], commandArguments: IArgument[]): Argv.Command {
        ...
        const commandDefinition = new Argv.Command(command.name, command.description, { sibling: aggregation?.command, alias: this.getAlias(command) })
